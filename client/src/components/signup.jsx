  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/auth/signup`, {
        ...
        body: JSON.stringify({ name: `${first} ${last}`.trim(), email, password })
      });
...
<path d="M42.6189 1.6525C40.3354 8.06058 39.3826 8.69942 32.5874 8.37906C38.9954 10.6627 39.6343 11.6154 39.3139 18.4106C41.5975 12.0025 42.5502 11.3637 49.3455 11.6841C42.9374 9.40048 42.2986 8.44777 42.6189 1.6525Z" fill="#44087D"/>
        ...
        <form className="signup-form" onSubmit={onSubmit}>
          <div className="name-row">
            <input type="text" placeholder="Enter first name" value={first} onChange={(e)=>setFirst(e.target.value)} required />
            <input type="text" placeholder="Enter last name" value={last} onChange={(e)=>setLast(e.target.value)} required />
          </div>
          <input style={{width:"95%"}} type="email" placeholder="Enter email ID" value={email} onChange={(e)=>setEmail(e.target.value)} required />
          <div style={{ position:'relative', width:'95%' }}>
            <input style={{width:"93%", paddingRight:44}} type={showPassword ? 'text' : 'password'} placeholder="Enter password" value={password} onChange={(e)=>setPassword(e.target.value)} required minLength={6} />
            <button
              type="button"
              aria-label={showPassword ? 'Hide password' : 'Show password'}
              onClick={() => setShowPassword(v=>!v)}
              style={{ position:'absolute', right:8, top:'50%', transform:'translateY(-50%)', background:'transparent', border:'none', cursor:'pointer', color:'#44087D', fontWeight:600 }}
            >{showPassword ? 'Hide' : 'Show'}</button>
          </div>
          <div style={{ position:'relative', width:'95%' }}>
            <input style={{width:"93%", paddingRight:44}} type={showConfirm ? 'text' : 'password'} placeholder="Confirm password" value={confirm} onChange={(e)=>setConfirm(e.target.value)} required minLength={6} />
            <button
              type="button"
